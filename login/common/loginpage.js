...
import * as helper from "../chrome/helpers";

const LARGE_SCORE_VALUE = 1000;

let SERVER_HINTS = {};

function setServerHints(hints: Object) {
  SERVER_HINTS = hints;
}

function evaluateServerHintsForEntity(type, fieldDict) {
  if (!SERVER_HINTS) {
    return 0;
  }
...
// login form is found.
function guessLoginForm(hints: Object) {
  function loginFormScoreFunc(formDict: Object) {
...
function isInputPresentAndVisible(name: string) {
...
function isLoginPageForService(service: Object) {
  var serviceHost = domain.getCanonicalHost(service.clientData.loginUrl);
  var curHost = domain.getCanonicalHost(window.location.href);
...
function fillLoginForm(formData: FormData) {
  ...
  var formDomain = domain.getCanonicalHost(formData.loginUrl);
  var pageDomain = domain.getCanonicalHost(document.URL);
  ...
  setTimeout(function() {
    ...
    var sendKeyEvents = function(inputId) {
      ...
      if (!el) {
        console.error("could not find element I expected to be in DOM: ", inputId);
        return;
      }
...
type FormData = any // TODO(tom)

function guessAndFillLoginForm(formData: FormData) {
...
export { guessUsernameField, guessPasswordField, getLoginForm, guessLoginForm,
         isLoginPageForService, isLoginPage, isLoginForm, fillLoginForm,
         setServerHints, guessAndFillLoginForm }
