...
client.initRemoteCalls('background',['generatePassword', 'getLoginState']);
...
if (window.location.hostname.match(/(^|\.)passopolis\.com$/) ||
    ...
    cs.sendMessageToBackground('getLoginState', function (message) {
      var identity = message.data.identity;
      if (identity != undefined) {
        setTimeout(function () {
          window.postMessage({type: "logged_in", uid: message.data.identity.uid}, window.location.origin);
        },2000);
      }
    });
